...
class MySchema(SwaggerAutoSchema):
    ...
    def get_default_responses(self):
        ...
        if action in ["create", "bulk_create"]:
...
class BaseViewSet(DjangoViewSet):
    ...
    @classmethod
    def as_bulk_create(cls):
        return cls.as_view({"post": "bulk_create"})
...
class BulkCreateModelMixin(BaseViewSet):
    @action(detail=False, methods=["POST"])
    def bulk_create(self, request, *args, **kwargs):
        try:
            ...
            for item in self.request.data:
                serializer = serializer_class(context=serializer_context, data=item)
                serializer.is_valid(raise_exception=True)
                serializer.save()
                instances.append(serializer.instance)
            return responses.Created(message=self.get_bulk_create_message(instances))
        except responses.BadRequest as response:
            return response
